// apps/web/src/pages/sharing/SharedReceiptSummaryPage.tsx
import { useEffect, useMemo, useState } from 'react'
import { Link, useParams } from 'react-router-dom'
import { getSharedReceiptSummary } from '../../services/sharing'
import type { ReceiptSummaryItem, ReceiptSummaryResponse } from '../../services/receipts'
...
export function SharedReceiptSummaryPage() {
	const { ownerId = '', receiptId = '' } = useParams<{ ownerId: string; receiptId: string }>()
	...
	useEffect(() => {
		...
		async function loadReceipt() {
			if (!ownerId || !receiptId) {
				setErrorMessage('Missing receipt information in the URL.')
				setIsLoading(false)
				return
			}
			...
			try {
				...
				const response = await getSharedReceiptSummary(ownerId, receiptId)
				if (isMounted) {
					setReceipt(response)
				}
			} catch (error) {
	...
	const receiptItems: ReceiptSummaryItem[] = receipt?.receiptItems ?? []
		...
		<main className="min-vh-100 bg-light py-5">
			<div className="container">
				<div className="d-flex justify-content-between align-items-start flex-wrap gap-3 mb-4">
					<div>
						<p className="text-uppercase text-secondary fw-semibold mb-2">Shared receipt summary</p>
						<h1 className="h2 mb-1">{receipt?.merchantName || 'Receipt detail'}</h1>
						<p className="text-secondary mb-0">{receipt?.receiptDate || 'No receipt date'}</p>
					</div>
					<Link to={`/shared/${encodeURIComponent(ownerId)}`} className="btn btn-outline-secondary">
						Back to receipts
					</Link>
				</div>
				...
				<div className="row g-4">
					<div className="col-12 col-lg-5">
						<div className="p-4 bg-white border rounded-4 shadow-sm h-100 d-grid gap-3" style={{ gridTemplateRows: 'auto auto 1fr' }}>
							...
							<div className="p-3 bg-light border rounded-3 small text-secondary align-self-start">
								You have read-only access to this shared receipt.
							</div>
						</div>
					</div>
						...
						<div className="p-4 p-md-5 bg-white border rounded-4 shadow-sm h-100">
							<div className="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-4">
								...
								<div className="d-flex flex-wrap gap-2 align-items-center">
									<span className="badge text-bg-info text-dark">{receipt?.category || 'Other'}</span>
							...
							<div className="p-4 bg-light border rounded-4 d-flex justify-content-between align-items-center">
								...
								<div className="text-end text-secondary small">
									<div>{receipt?.merchantName || 'Shared receipt'}</div>
